...
def run(formula_original, truth_table = ""):
	...
	atomics_qubits = [] # aqui estarão armazenados os indices de qubits atômicos
	...
	make_oracle(size, formula_use, circuit, qubits, cirq.LineQubit(13))

	# Embed the oracle into a quantum circuit implementing Grover's algorithm.
	#circuit = make_grover_circuit(input_qubits, output_qubit)
	#print('Circuit:')
	#print(circuit)
...
def make_oracle(size, formula_use, circuit, qubits, output_qubit):
	...
	reverse = circuit[::-1]
	circuit.append(cirq.CNOT(qubits[indexator[0]], output_qubit))
	circuit.append(cirq.CZ(qubits[indexator[0]], output_qubit))
...
#run("(((A ∨ B)∧ (A→ C)∧ (B→ C))→C) ∨ (((D ∨ E)∧ (D → G)∧ (E→ C))→G)")
#run("A&B&C&D&E&G&H")
run("A|B|C|D|E|G|H")